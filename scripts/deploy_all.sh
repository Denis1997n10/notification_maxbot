...
if [[ "$env" == "prod" ]]; then
  read -rp "Type deploy-prod to continue: " c
  [[ "$c" == "deploy-prod" ]] || exit 1
fi
...
read -rp "Confirm terraform apply? [yes/no]: " ok
[[ "$ok" == "yes" ]]
...
if [[ "$env" == "dev" ]]; then
  ENV=local PYTHONPATH="$PWD/backend/src" python3 backend/src/cli/seed_demo_data.py || true
fi
