...
(
  cd "$ROOT_DIR/frontend/public-site"
  npm ci
  VITE_PUBLIC_API_BASE_URL="$API_BASE_URL" npm run build >/dev/null
)

(
  cd "$ROOT_DIR/frontend/admin-panel"
  npm ci
  VITE_ADMIN_API_BASE_URL="$API_BASE_URL" npm run build >/dev/null
)
