...
from application.services import FeatureFlagService, NotificationService
...
from domain.value_objects.enums import AdminRole, EventType, Source
...
from infrastructure.ydb.repositories import (
    ...
    YdbPublicPageCacheRepository,
    YdbSubjectRepository,
    YdbSubscriptionRepository,
    YdbUserFeatureFlagRepository,
    YdbUserRepository,
)
...
class PublicService:
    ...
    def get_entrance_page(self, public_code: str):
        s = self.subjects.get_by_public_code(public_code)
        if not s:
...
class AdminService:
    def __init__(self, admin_repo: YdbAdminUserRepository, perm_repo: YdbAdminPermissionRepository, subjects: YdbSubjectRepository, session, jwt_secret: str):
...
class BotService:
    def __init__(self, users, subjects, subscriptions, sub_uc, list_uc, disable_uc, max_channel):
        self.users = users
        self.subjects = subjects
...
def build_container() -> AppContainer:
    ...
    admin_perm = YdbAdminPermissionRepository(session)
    flags = YdbFeatureFlagRepository(session)
    user_flags = YdbUserFeatureFlagRepository(session)
    ...
    return AppContainer(
        ...
        admin_service=AdminService(admin_users, admin_perm, subjects, session, os.getenv("ADMIN_JWT_SECRET", "dev-secret")),
